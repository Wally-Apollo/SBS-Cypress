...
context('Sales -> Cash Drop', () => {
    ...
    it('Search Cash Drop', ()=> {
        ...
        //Click Cash Drop from menu list
        navigateToSubModule('Cash Drop');

        //Validate that there will be no Error message displayed
        validateModule();
        
        cy.fixture('sales/cash_drop/search_cash_drop_list_data').then((data) => {
    ...
    it('Validation of Show Cash Drop page', () => {
        ...
        cy.get('tbody').find("tr").then((row) =>{
            for(let i = 0; i< row.length; i++){
                ...
                cy.get('h3').contains('Show Cash Drop');
                ...
                cy.get('.btn').contains('Back to Cash Drop List').click();
            }
        })
    })
})
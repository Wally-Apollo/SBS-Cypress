        ...
        if(data[key] != "") { 
            ...
            cy.get('table tbody').then($tbody=>{
                ...
                cy.wrap(check).then(value=>{
                    if(value>0){
                        cy.get('[data-cy="cash-drop-table"]').should('have.descendants', 'td');
                    }
                })
            })
...
function validateModule(){
    ...
    cy.get('[data-cy="title"]').contains('Cash Drop List');
    ...
    cy.get('[data-cy="cash-drop-table"]').contains('Business Date');
    cy.get('[data-cy="cash-drop-table"]').contains('POS');
    cy.get('[data-cy="cash-drop-table"]').contains('Shift');
    cy.get('[data-cy="cash-drop-table"]').contains('Amount');
